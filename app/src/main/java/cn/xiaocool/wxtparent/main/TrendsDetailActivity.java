...
public class TrendsDetailActivity extends Activity {
                ...
                case WORK_PRAISE_KEY:
                    if (msg.obj != null) {
                        try {
                            ...
                            ToastUtils.ToastShort(context, result);
                            if (state.equals(CommunalInterfaces._STATE)) {
                                new SpaceRequest(context, handler).getCircleList(user.getChildId(), null, "1", "1", "1", GET_CIRCLE_LIST_KEY);
                            }
                ...
                case DEL_WORK_PRAISE_KEY:
                    if (msg.obj != null) {
                        try {
                            ...
                            String result = json.getString("data");
                            if (state.equals(CommunalInterfaces._STATE)) {
                                ToastUtils.ToastShort(context, "已取消");
                                new SpaceRequest(context, handler).getCircleList(user.getChildId(), null, "1", "1", "1", GET_CIRCLE_LIST_KEY);
                            }else
                            {
                                ToastUtils.ToastShort(context, result);
    ...
    private void initview() {
        ...
        ImageLoader.getInstance().displayImage(NetBaseConstant.NET_CIRCLEPIC_HOST+homeworkData.getMemberImg(),item_head);
        ...
        btn_parent_send = (TextView) findViewById(R.id.btn_parent_send);
        ...
        btn_parent_send.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (parent_warn_comment_edit.getText().length() > 0) {
                    //获取到需要上传的参数
                    new SpaceRequest(context, handler).send_remark(homeworkData.getId(), String.valueOf(parent_warn_comment_edit.getText()), "1");
                } else {
        ...
        if (homeworkData.getComment().size()>=1){
            ...
            //加载数据
            news_group_comment_list.setAdapter(new CommentAdapter(context,homeworkData.getComment()));
...
//                holder.img_gridview.setVisibility(View.GONE);
    ...
    private void workPraise(String workBindId) {
        ...
        new SpaceRequest(context, handler).Praise(workBindId, WORK_PRAISE_KEY,"1");
    }

    // 取消点赞
    private void delPraise(String workBindId) {
        new SpaceRequest(context, handler).DelPraise(workBindId, DEL_WORK_PRAISE_KEY,"1");
    }
    /**
     * 返回 普通资讯 JSON
     *
