...
import cn.xiaocool.wxtparent.adapter.ChatInfoGridAdapter;
...
public class ChatInfoActivity extends BaseActivity {
    ...
    private void setAdapter() {
        if (pAdapter==null){
            pAdapter = new ChatInfoGridAdapter(pList,context,1);
        }else {
            pAdapter.notifyDataSetChanged();
        }
        if (tAdapter==null){
            tAdapter = new ChatInfoGridAdapter(tList,context,2);
        }else {
            ...
            switch (requestCode) {
                ...
                case 101:
                    if (data != null) {
                        ...
                        if(inviderid==null){
                            ToastUtils.ToastShort(context,"邀请的人已在群组中，无需重复邀请");
                        }else{
                            inviderid = inviderid.substring(5, inviderid.length());
                            str = str.substring(5, str.length());
                            invidePersons(str);
