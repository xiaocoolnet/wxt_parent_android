package cn.xiaocool.wxtparent.presenter;
...
public class ImageBrowsePresenter {
    ...
    public void saveImage(){
        ...
        String imagesUrl = NetBaseConstant.NET_CIRCLEPIC_HOST+imageUrl;
        if(type.equals("6")){
            imagesUrl = "http://wxt.xiaocool.net/data/upload/" + imageUrl;
        }
        Picasso.with(view.getMyContext()).load(imagesUrl).into(new Target() {
            @Override
            public void onBitmapLoaded(Bitmap bitmap, Picasso.LoadedFrom from) {
                // 创建目录
                File appDir = new File(Environment.getExternalStorageDirectory(), "zxy_Image");
                if (!appDir.exists()) {
                    appDir.mkdir();
                }
